"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.DefaultPackageManager = void 0;exports.getExtensionTempFolder = getExtensionTempFolder;
...
var _glob = await jitiImport("glob");
var _ignore = _interopRequireDefault(await jitiImport("ignore"));
var _minimatch = await jitiImport("minimatch");
var _semver = await jitiImport("semver");
...
function isExactNpmVersion(version) {
  return (0, _semver.valid)(version ?? "") !== null;
}

function getNpmVersionRange(version) {
  return version ? (0, _semver.validRange)(version) ?? undefined : undefined;
}
...
function getExtensionTempFolder(agentDir) {
  const tempFolder = (0, _nodePath.join)(agentDir, "tmp", "extensions");
  (0, _nodeFs.mkdirSync)(tempFolder, { recursive: true, mode: 0o700 });
  (0, _nodeFs.chmodSync)(tempFolder, 0o700);
  return tempFolder;
}
...
function addIgnoreRules(ig, dir, rootDir) {
  ...
  for (const filename of IGNORE_FILE_NAMES) {
    ...
    if (!(0, _nodeFs.existsSync)(ignorePath)) continue;
    try {
...
{
  const files = [];
  if (!(0, _nodeFs.existsSync)(dir)) return files;
...
{
  const entries = [];
  if (!(0, _nodeFs.existsSync)(dir)) return entries;
...
function matchesAnyPattern(filePath, patterns, baseDir) {
  ...
  return patterns.some((pattern) => {
    ...
    if (
    (0, _minimatch.minimatch)(rel, normalizedPattern) ||
    ...
    return (
      (0, _minimatch.minimatch)(parentRel, normalizedPattern) ||
...
class DefaultPackageManager {
  ...
  onMissing)
  {
    for (const { pkg, scope } of sources) {
      ...
      if (parsed.type === "npm") {
        ...
        const needsInstall =
        !(0, _nodeFs.existsSync)(installedPath) || !(await this.installedNpmMatchesConfiguredVersion(parsed, installedPath));
        if (needsInstall) {
  ...
  async installedNpmMatchesConfiguredVersion(source, installedPath) {
    ...
    return source.range ? (0, _semver.satisfies)(installedVersion, source.range) : true;
  }
  ...
  getInstalledNpmVersion(installedPath) {
    ...
    if (!(0, _nodeFs.existsSync)(packageJsonPath)) return undefined;
    try {
      const content = (0, _nodeFs.readFileSync)(packageJsonPath, "utf-8");
      const pkg = JSON.parse(content);
      return pkg.version;
  ...
  async getLatestNpmVersion(packageSpec, range) {
    ...
    if (Array.isArray(parsed)) {
      ...
      const latest = range ? (0, _semver.maxSatisfying)(versions, range) : [...versions].sort(_semver.rcompare)[0];
  ...
  getPackageManagerName() {
    ...
    return packageManagerCommand ? (0, _nodePath.basename)(packageManagerCommand).replace(/\.(cmd|exe)$/i, "") : "";
  }
  ...
  async removeGit(source, scope) {
    const targetDir = this.getGitInstallPath(source, scope);
    if (!(0, _nodeFs.existsSync)(targetDir)) return;
  ...
  ensureNpmProject(installRoot) {
    ...
    (0, _paths.markPathIgnoredByCloudSync)(installRoot);
  ...
  getNpmInstallRoot(scope, temporary) {
    ...
    return (0, _nodePath.join)(this.agentDir, "npm");
  }
  ...
  getManagedNpmInstallPath(source, scope) {
    ...
    return (0, _nodePath.join)(this.agentDir, "npm", "node_modules", source.name);
  }

  getLegacyGlobalNpmInstallPath(source) {
    try {
      return this.getPnpmGlobalPackagePath(source.name) ?? (0, _nodePath.join)(this.getGlobalNpmRoot(), source.name);
    } catch {
      return undefined;
    }
  }
  ...
  getNpmInstallPath(source, scope) {
    ...
    if (scope !== "user" || (0, _nodeFs.existsSync)(managedPath)) {
      return managedPath;
    }
    const legacyPath = this.getLegacyGlobalNpmInstallPath(source);
    return legacyPath && (0, _nodeFs.existsSync)(legacyPath) ? legacyPath : managedPath;
  }
  ...
  resolveManagedPath(root, ...parts) {
    const resolvedRoot = (0, _nodePath.resolve)(root);
    const resolvedPath = (0, _nodePath.resolve)(resolvedRoot, ...parts);
    if (resolvedPath !== resolvedRoot && !resolvedPath.startsWith(`${resolvedRoot}${_nodePath.sep}`)) {
  ...
  resolvePath(input) {
    return (0, _paths.resolvePath)(input, this.cwd, { homeDir: getHomeDir(), trim: true });
  }

  resolvePathFromBase(input, baseDir) {
    return (0, _paths.resolvePath)(input, baseDir, { homeDir: getHomeDir(), trim: true });
  }
  ...
  projectBaseDir)
  {
    ...
    collectAncestorAgentsSkillDirs(this.cwd).filter((dir) => (0, _nodePath.resolve)(dir) !== (0, _nodePath.resolve)(userAgentsSkillsDir)) :
    [];
    ...
    for (const agentsSkillsDir of projectAgentsSkillDirs) {
      const agentsBaseDir = (0, _nodePath.dirname)(agentsSkillsDir); // the .agents directory
    ...
    const userAgentsBaseDir = (0, _nodePath.dirname)(userAgentsSkillsDir);
  ...
  collectFilesFromPaths(paths, resourceType) {
    ...
    for (const p of paths) {
      if (!(0, _nodeFs.existsSync)(p)) continue;
  ...
  spawnCommand(command, args, options) {
    const env = getEnv();
    return (0, _childProcess.spawnProcess)(command, args, {
      cwd: options?.cwd,
...
}exports.DefaultPackageManager = DefaultPackageManager; /* v9-00fe03ec308eb142 */
