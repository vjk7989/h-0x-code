"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.default = promptUrlWidgetExtension;var _promises = await jitiImport("node:fs/promises");
...
function resolveDraftPath(cwd, target) {
  if (target === "~") return (0, _nodeOs.homedir)();
  if (target.startsWith("~/")) return (0, _nodePath.resolve)((0, _nodeOs.homedir)(), target.slice(2));
  return (0, _nodePath.resolve)(cwd, target);
}

async function readAdvisoryRefFromDraft(cwd, target) {
  try {
    const content = await (0, _promises.readFile)(resolveDraftPath(cwd, target), "utf8");
...
function formatAdvisoryDetail(advisory) {
...
async function fetchAdvisoryMetadata(pi, cwd, target) {
...
function promptUrlWidgetExtension(pi) {
  const setWidget = (ctx, match, metadata) => {
    ctx.ui.setWidget("prompt-url", (_tui, thm) => {
      ...
      container.addChild(new _piCodingAgent.DynamicBorder((s) => thm.fg("muted", s)));
      container.addChild(new _piTui.Text(lines.join("\n"), 1, 0));
      return container;
    });
  };

  const applySessionName = (ctx, match, metadata) => {
  ...
  const updatePromptContext = (ctx, match) => {
    setWidget(ctx, match);
