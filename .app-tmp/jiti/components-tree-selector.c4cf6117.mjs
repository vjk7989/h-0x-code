"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.TreeSelectorComponent = void 0;var _piTui = await jitiImport("@earendil-works/pi-tui");
...
class TreeList {
  ...
  render(width) {
    ...
    lines.push(
      (0, _piTui.truncateToWidth)(
        _theme.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredNodes.length})${this.getStatusLabels()}`),
        width
      )
    );
...
class TreeHelp {
  ...
  render(width) {
    ...
    for (const item of items) {
      ...
      (0, _piTui.visibleWidth)(`${indent}${item}`) <= availableWidth ?
      `${indent}${item}` :
      item;
      if (!currentLine || (0, _piTui.visibleWidth)(candidate) <= availableWidth) {
        currentLine = candidate;
        continue;
      }

      lines.push(...(0, _piTui.wrapTextWithAnsi)(currentLine.trimEnd(), availableWidth));
      currentLine = (0, _piTui.visibleWidth)(`${indent}${item}`) <= availableWidth ? `${indent}${item}` : item;
    }
    ...
    return lines.map((line) => _theme.theme.fg("muted", line));
  }
}
...
function formatHelpKeys(keybindings) {
  ...
  for (const keybinding of keybindings) {
    const key = (0, _piTui.getKeybindings)().getKeys(keybinding)[0];
    if (key !== undefined) keys.push(key);
  }
  if (keys.length === 0) return "";

  return (0, _keybindingHints.formatKeyText)(compactRawKeys(keys)).
...
class LabelInput {
  ...
  render(width) {
    ...
    lines.push(
      (0, _piTui.truncateToWidth)(
        `${indent}${(0, _keybindingHints.keyHint)("tui.select.confirm", "save")}  ${(0, _keybindingHints.keyHint)("tui.select.cancel", "cancel")}`,
        width
      )
    );
    return lines;
...
}exports.TreeSelectorComponent = TreeSelectorComponent; /* v9-a3d9329c7134eb8c */
