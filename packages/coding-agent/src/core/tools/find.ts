			...
			return new Promise((resolve, reject) => {
				...
				(async () => {
					try {
						...
						const makeResult = (paths: string[], resultLimitReached: boolean) => {
						...
						if (customOps?.glob) {
							...
							settle(() => resolve(makeResult(relativized, relativized.length >= effectiveLimit)));
							return;
						}
						...
						if (process.platform === "win32" && pattern.includes("/")) {
							...
							const relativized = globbed.slice(0, effectiveLimit).map(toPosixPath);
							...
							settle(() => resolve(makeResult(relativized, globbed.length >= effectiveLimit)));
							return;
						}
