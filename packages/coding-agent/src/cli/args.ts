	...
	const extensionFlagsText =
		extensionFlags && extensionFlags.length > 0
			? `\n${chalk.bold("Extension CLI Flags:")}\n${extensionFlags
					...
					.join("\n")}\n`
			: "";
	const titleLine = APP_TITLE === APP_NAME ? APP_NAME : `${APP_TITLE} (${APP_NAME})`;
	const taglineLine = APP_TAGLINE ? `\n${APP_TAGLINE}` : "";
	const creditLine = APP_CREDIT_LINE ? `\n${APP_CREDIT_LINE}` : "";
	console.log(`${chalk.bold(titleLine)} - AI coding assistant with read, bash, edit, write tools${taglineLine}${creditLine}
...
${chalk.bold("Commands:")}
  ...
  ${APP_NAME} config path               Show H-0x config file paths
  ${APP_NAME} provider list             List configured H-0x providers
