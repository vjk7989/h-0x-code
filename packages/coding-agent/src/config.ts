...
function getNodeModulesPackageRoot(): string | undefined {
	const packageDir = getPackageDir();
	const path = process.platform === "win32" || packageDir.includes("\\") ? win32 : { basename, dirname };
	const parent = path.dirname(packageDir);
	if (path.basename(parent).startsWith("@") && path.basename(path.dirname(parent)) === "node_modules") {
...
function getGlobalPackageRoots(method: InstallMethod, _packageName: string, npmCommand?: string[]): string[] {
	switch (method) {
		...
		case "pnpm": {
			...
			const packageRoot = getNodeModulesPackageRoot();
			if (root) return [root, dirname(root), packageRoot].filter((x): x is string => !!x);
			const match = /^(.*[\\/]global[\\/][^\\/]+)[\\/]\.pnpm[\\/]/.exec(getPackageDir());
			return [match?.[1], packageRoot].filter((x): x is string => !!x);
		}
		case "yarn": {
			const dir = readCommandOutput("yarn", ["global", "dir"]);
			const packageRoot = getNodeModulesPackageRoot();
			return dir
				? [dir, join(dir, "node_modules"), packageRoot].filter((x): x is string => !!x)
				: [packageRoot].filter((x): x is string => !!x);
		}
		case "bun": {
...
export const APP_TITLE: string = pkg.piConfig?.title || (piConfigName ? APP_NAME : "pi");
export const APP_TAGLINE: string | undefined = pkg.piConfig?.tagline;
export const APP_CREDIT_LINE: string | undefined = pkg.piConfig?.creditLine;
