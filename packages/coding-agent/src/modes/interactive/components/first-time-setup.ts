...
import { APP_CREDIT_LINE, APP_NAME, APP_TAGLINE, APP_TITLE } from "../../../config.ts";
...
export class FirstTimeSetupComponent extends Container {
	...
	private update(): void {
		...
		this.addChild(new Spacer(1));
		const welcome = `Welcome to ${APP_TITLE} (${APP_NAME}), the minimal coding agent.`;
		const subtitle = [APP_TAGLINE, APP_CREDIT_LINE].filter((line): line is string => Boolean(line)).join("\n");
